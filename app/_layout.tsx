      ...
      try {
        ...
        if (isLoaded) {
          if (isSignedIn && userId) {
            ...
            if (!localToken) {
              console.log(
                "No hay token local, intentando obtener token de Clerk",
              );
              try {
                // No hay token local, intentamos obtener el de Clerk
                console.log(
                  "Intentando obtener token de Clerk con múltiples métodos...",
                );
                ...
                if (!clerkToken) {
                  try {
                    ...
                    console.log(
                      "Método 2 - template org-public-key:",
                      clerkToken ? "Éxito" : "Fallido",
                    );
                  } catch (error) {
                ...
                if (clerkToken) {
                  ...
                  try {
                    ...
                    if (response.ok) {
                      console.log(
                        "Usuario recuperado del backend usando Clerk ID",
                      );
                      ...
                      if (userData && userData.user) {
                        console.log(
                          "Guardando datos de usuario en AsyncStorage",
                        );
                      ...
                      console.log(
                        "No se pudo recuperar el usuario del backend. Status:",
                        response.status,
                      );
                      try {
                        ...
                        if (
                          ...
                          console.log(
                            "Error de autorización detectado, es posible que la API requiera configuración",
                          );
                        }
                      } catch (e) {
                    ...
                    console.error(
                      "Error intentando sincronizar con backend:",
                      error,
                    );
                  }
                } else {
            ...
            if (localToken && !isSignedIn) {
              console.log(
                "Limpiando token local porque no hay sesión en Clerk",
              );
